...
- **Secure auth**: OS keyring by default (`zalando/go-keyring`), with
  environment-variable fallback (`BT_TOKEN`, `BT_EMAIL`, `BT_USERNAME`,
  `BT_PAT`, and legacy `BT_APP_PASSWORD`) for CI.
...
```bash
bt auth login                       # guided sign-in; can open browser for token creation
bt context list
